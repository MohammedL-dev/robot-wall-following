...
void loop() {
  ...
  if (distanceFront == 0 || distanceRight == 0) {
    stopMotors();
  ...
  if (distanceFront < 5) {
    ...
    moveForward();
    setColor(0, 0, 255); //MS7O HADA. 7TITO RIR 7IT CHACK
  }
...
void steerRight() {
  analogWrite(MOTOR_A_EN, baseSpeedA / 2); 
  analogWrite(MOTOR_B_EN, baseSpeedB);
  setMotorsForward();
}
...
void steerLeft() {
  analogWrite(MOTOR_A_EN, baseSpeedA);     
  analogWrite(MOTOR_B_EN, baseSpeedB / 2);
  setMotorsForward();
}
